...
def plot_firing_rates(dataset, constvar, comparevar, means, outplotdir, colmap="tab20", padcoords=[0.08,0.08], padsize=[0.90,0.86]):
    '''
    ...
    the firing rates for all the concentrations and all the GRNs in the dataset.
    ...
    Args:
        ...
        comparevar (list(str)): list of dataseries you want to put on one plot.
        ...
        colmap (str): name of the python colour palette used, default to tab20.
    ...
    for x in comparevar:
        ...
        for i,chan in enumerate(channels):
            ...
            plt.plot(dataset.times, yvals[chan], label=x+" "+chan, linestyle=lines[i])
    # set the legend and labels
    plt.legend()
    plt.xlabel("time [ms]")
    plt.ylabel("Mean Firing Rate [/s]")
    plt.title("Average Firing rate over beeID/Sensillum for "+constvar+", bins = "+bintime+" ms")
    for ext in [".pdf",".png"]:
